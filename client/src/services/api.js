const API_BASE_URL = "https://home-expense-tracker-one.vercel.app" || 'http://localhost:5000/api';
...
export const api = {
  ...
  // Providers
  getProviders: (status) => request(`/providers${status ? `?status=${status}` : ''}`),
  ...
  getDailyLogs: (providerId, month) =>
    request(`/daily-logs?providerId=${providerId}${month ? `&month=${month}` : ''}`),
  upsertDailyLog: (payload) => request('/daily-logs', { method: 'POST', body: JSON.stringify(payload) }),
  bulkUpsertDailyLogs: (payload) => request('/daily-logs/bulk', { method: 'POST', body: JSON.stringify(payload) }),
  ...
  recordPayment: (payload) => request('/payments', { method: 'POST', body: JSON.stringify(payload) }),
