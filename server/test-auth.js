...
async function testAuth() {
  ...
  console.log(
    "1. No token request status:",
    noTokenRes.status,
    noTokenRes.status === 401 ? "PASSED (401 Unauthorized) ✅" : "FAILED ❌"
  );
  ...
  console.log(
    "2. User login:",
    userLogin.success ? "PASSED ✅" : "FAILED ❌",
    "Role:",
    userLogin.user?.role
  );
  ...
  console.log(
    "3. User role viewing providers:",
    userViewRes.status,
    userViewRes.status === 200 ? "PASSED (200 OK) ✅" : "FAILED ❌"
  );
  ...
  console.log(
    "4. User role attempting to add provider:",
    userAddRes.status,
    userAddRes.status === 403 ? "PASSED (403 Forbidden) ✅" : "FAILED ❌",
    userAdd.message
  );
  ...
  console.log(
    "6. Admin role adding provider:",
    adminAddRes.status,
    adminAddRes.status === 201 ? "PASSED (201 Created) ✅" : "FAILED ❌",
    adminAdd.data?.name
  );
